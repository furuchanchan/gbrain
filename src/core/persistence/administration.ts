...
export async function runPersistenceAdministration(engine: BrainEngine, operation: PersistenceAdminOperation,
  ...
  if (operation === 'writer_status') {
    ...
    // int8 columns serialize as BigInt under postgres — cast to ::text like
    // every other diagnostics projection so `writer status` output is
    // JSON-safe on Postgres engines (#5342).
